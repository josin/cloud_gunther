...
describe MacroProcesor do
  ...
  describe "process_macros method" do
    it "calls binary macro" do
      MacroProcesor.should_receive(:binary_macro)
      MacroProcesor.process_macros("Lorem ipsum {{BINARY}} dolor sit amet.", mock_context)
    end

    it "calls binary and inputs macros" do
      MacroProcesor.should_receive(:inputs_macro)
      MacroProcesor.should_receive(:binary_macro)
      MacroProcesor.process_macros("Lorem ipsum {{INPUTS}} dolor {{BINARY}} sit amet.", mock_context)
    end
    
    it "doesn't raise an error when macro doesn't exist" do
      lambda { MacroProcesor.process_macros("Lorem ipsum {{FOO}} dolor {{BAR}} sit amet.", mock_context) }.should_not raise_error
    end
    
    it "raises error when unsuitable context provided" do
      lambda { MacroProcesor.process_macros("Hello {{FOO}} World!", mock("text")) }.should raise_error
    end
    
    it "returns inputs string even if no macros provided" do
      str = "Lorem ipsum dolor sit amet."
      MacroProcesor.process_macros(str, mock_context).should eq(str)
    end
  ...
  describe "binary macro" do
    it "returns algorithm binaries file name" do
      ...
      MacroProcesor.send(:binary_macro, mock_context).should eq("file")
    end
  end
  ...
  describe "inputs macro" do
    it "returns inputs" do
      mock_context.stub(:inputs).and_return("inputs")
      MacroProcesor.send(:inputs_macro, mock_context).should eq("inputs")
    end
  end
  ...
  describe "instances count macro" do
    it "returns instances count" do
      ...
      MacroProcesor.send(:instances_count_macro, mock_context).should eq(3)
    end
  end
  ...
  describe "instance id macro" do
    it "returns instance_id" do
      ...
      MacroProcesor.send(:instance_id_macro, mock_context).should eq(1)
    end
  end

  describe "cloud user macro" do
    it "returns cloud user" do
      MacroProcesor.send(:cloud_user_macro, mock_context).should eq("euca")
    end
  end
  ...
  describe "unix uid macro" do
    it "returns context user's unix uid" do
      ...
      MacroProcesor.send(:unix_uid_macro, mock_context).should eq(1000)
    end
  end
end